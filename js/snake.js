...
Elm.Snake.make = function (_elm) {
   ...
   if (_elm.Snake.values) return _elm.Snake.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   ...
   var _op = {};
   var sounds = $Signal.mailbox("");
   var soundSignal = sounds.signal;
   var wrapInt = F3(function (input,min,max) {
      var size = max - min;
      return _U.cmp(input,min) < 0 ? input + size : _U.cmp(input,
      max) > -1 ? input - size : input;
   });
   var getTail = function (game) {
      var _p0 = game.snake;
      if (_p0.ctor === "::") {
            return _p0._1;
         } else {
   ...
   var tickSnake = function (game) {
      return _U.update(game,
      {snake: A2($List.take,game.length,game.snake)});
   };
   var collisionTest = F2(function (testPoint,candidates) {
      return A2($List.any,
      function (point) {
         return _U.eq(testPoint,point);
      },
      candidates);
   });
   var randomInt = F3(function (min,max,seed) {
      return A2($Random.generate,A2($Random.$int,min,max),seed);
   });
   var defaultBonus = {point: $Maybe.Nothing,ticks: -1};
   var defaultPoint = {x: 0,y: 0};
   var getHead = function (game) {
      var _p1 = game.snake;
      if (_p1.ctor === "::") {
            return _p1._0;
         } else {
   ...
   var Bonus = F2(function (a,b) {
      return {point: a,ticks: b};
   });
   ...
   var Up = {ctor: "Up"};
   var changeDirection = function (game) {
      var y = game.arrows.y;
      var x = game.arrows.x;
      return _U.cmp(x,0) > 0 && !_U.eq(game.direction,
      Left) ? Right : _U.cmp(x,0) < 0 && !_U.eq(game.direction,
      Right) ? Left : _U.cmp(y,0) > 0 && !_U.eq(game.direction,
      Down) ? Up : _U.cmp(y,0) < 0 && !_U.eq(game.direction,
      Up) ? Down : game.direction;
   };
   var GameOver = {ctor: "GameOver"};
   var Dead = function (a) {    return {ctor: "Dead",_0: a};};
   var Pause = {ctor: "Pause"};
   var Play = {ctor: "Play"};
   var NewGame = {ctor: "NewGame"};
   var Noop = {ctor: "Noop"};
   var triggerSounds = function (game) {
      var sendSound = function (sound) {
         return A2($Effects.map,
         $Basics.always(Noop),
         $Effects.task(A2($Signal.send,sounds.address,sound)));
      };
      return {ctor: "_Tuple2"
             ,_0: _U.update(game,{sounds: _U.list([])})
             ,_1: $Effects.batch(A2($List.map,sendSound,game.sounds))};
   };
   var StartTime = function (a) {
      return {ctor: "StartTime",_0: a};
   };
   var getStartTime = $Effects.task(A2($Task.map,
   StartTime,
   $TaskTutorial.getCurrentTime));
   var Window = function (a) {    return {ctor: "Window",_0: a};};
   var Space = function (a) {    return {ctor: "Space",_0: a};};
   var Tick = function (a) {    return {ctor: "Tick",_0: a};};
   var Wasd = function (a) {    return {ctor: "Wasd",_0: a};};
   var Arrows = function (a) {    return {ctor: "Arrows",_0: a};};
   var maxTicksToNextBonus = 400;
   var minTicksToNextBonus = 200;
   var resetBonus = function (game) {
      var _p2 = A3(randomInt,
      minTicksToNextBonus,
      maxTicksToNextBonus,
      game.seed);
      var ticks = _p2._0;
      var seed$ = _p2._1;
      var bonus$ = {point: $Maybe.Nothing,ticks: ticks};
      return _U.update(game,{seed: seed$,bonus: bonus$});
   };
   var bonusBlipMod = 10;
   var activeBonusTicks = 50;
   var deathTicks = 32;
   var deathFlashCount = deathTicks / 4 | 0;
   var tickDead = F2(function (game,count) {
      return _U.eq(count + 1,deathTicks) ? _U.update(game,
      {mode: GameOver}) : _U.update(game,{mode: Dead(count + 1)});
   });
   var tickFps = 20;
   var bonusScore = 5;
   var foodScore = 1;
   var foodEnergy = 4;
   var _p3 = {ctor: "_Tuple2",_0: 48,_1: 27};
   var gameWidth = _p3._0;
   var gameHeight = _p3._1;
   var defaultGame = {mode: NewGame
                     ,direction: Right
                     ,arrows: defaultPoint
                     ,snake: _U.list([{x: gameWidth / 2 | 0,y: gameHeight / 2 | 0}])
                     ,length: 2
                     ,food: defaultPoint
                     ,bonus: defaultBonus
                     ,seed: $Random.initialSeed(0)
                     ,score: 0
                     ,window: {ctor: "_Tuple2",_0: 0,_1: 0}
                     ,sounds: _U.list([])};
   var initialGame = {ctor: "_Tuple2"
                     ,_0: defaultGame
                     ,_1: getStartTime};
   var randomPoint = function (seed) {
      var _p4 = A3(randomInt,0,gameWidth - 1,seed);
      var x = _p4._0;
      var seed0 = _p4._1;
      var _p5 = A3(randomInt,0,gameHeight - 1,seed0);
      var y = _p5._0;
      var seed1 = _p5._1;
      return {ctor: "_Tuple2",_0: {x: x,y: y},_1: seed1};
   };
   var newFood = function (game) {
      var newFood$ = function (game) {
         var _p6 = randomPoint(game.seed);
         var point$ = _p6._0;
         var seed$ = _p6._1;
         var game$ = _U.update(game,{seed: seed$,food: point$});
         var bonusCollision = function () {
            var _p7 = game.bonus.point;
            if (_p7.ctor === "Just") {
                  return _U.eq(point$,_p7._0);
               } else {
                  return false;
               }
         }();
         return bonusCollision || A2(collisionTest,
         point$,
         game.snake) ? $Trampoline.Continue(function (_p8) {
            return newFood$(game$);
         }) : $Trampoline.Done(game$);
      };
      return $Trampoline.trampoline(newFood$(game));
   };
   var startGame = function (game) {
      return resetBonus(newFood(_U.update(defaultGame,
      {seed: game.seed,window: game.window,mode: Play})));
   };
   var changeGameMode = function (game) {
      var _p9 = game.mode;
      switch (_p9.ctor)
      {case "NewGame": return startGame(game);
         case "Play": return _U.update(game,{mode: Pause});
         case "Pause": return _U.update(game,{mode: Play});
         case "Dead": return game;
         default: return startGame(game);}
   };
   var tickFood = function (game) {
      return A2(collisionTest,
      game.food,
      game.snake) ? newFood(_U.update(game,
      {length: game.length + foodEnergy
      ,score: game.score + foodScore
      ,sounds: A2($List._op["::"],"food-eaten",game.sounds)})) : game;
   };
   var newBonus = function (game) {
      var newBonus$ = function (game) {
         var _p10 = randomPoint(game.seed);
         var point$ = _p10._0;
         var seed$ = _p10._1;
         var bonus$ = {point: $Maybe.Just(point$)
                      ,ticks: activeBonusTicks};
         var game$ = _U.update(game,{seed: seed$,bonus: bonus$});
         var foodCollision = _U.eq(point$,game.food);
         return foodCollision || A2(collisionTest,
         point$,
         game.snake) ? $Trampoline.Continue(function (_p11) {
            return newBonus$(game$);
         }) : $Trampoline.Done(game$);
      };
      return $Trampoline.trampoline(newBonus$(game));
   };
   var tickBonus = function (game) {
      var bonus$ = {point: game.bonus.point
                   ,ticks: game.bonus.ticks - 1};
      var _p12 = bonus$.point;
      if (_p12.ctor === "Just") {
            if (A2(collisionTest,_p12._0,game.snake))
            return resetBonus(_U.update(game,
               {length: game.length + foodEnergy
               ,score: game.score + bonusScore
               ,sounds: A2($List._op["::"],"bonus-eaten",game.sounds)}));
            else {
                  var _p13 = bonus$.ticks;
                  if (_p13 === 0) {
                        return resetBonus(game);
                     } else {
                        var sounds$ = _U.eq(A2($Basics._op["%"],
                        game.bonus.ticks,
                        bonusBlipMod),
                        0) ? A2($List._op["::"],
                        "bonus-tick-high",
                        game.sounds) : _U.eq(A2($Basics._op["%"],
                        game.bonus.ticks,
                        bonusBlipMod),
                        $Basics.round(bonusBlipMod / 2)) ? A2($List._op["::"],
                        "bonus-tick-low",
                        game.sounds) : game.sounds;
                        return _U.update(game,{bonus: bonus$,sounds: sounds$});
                     }
               }
         } else {
            var _p14 = bonus$.ticks;
            if (_p14 === 0) {
                  return newBonus(game);
               } else {
                  return _U.update(game,{bonus: bonus$});
               }
         }
   };
   var wrapPoint = function (input) {
      return {x: A3(wrapInt,input.x,0,gameWidth)
             ,y: A3(wrapInt,input.y,0,gameHeight)};
   };
   var moveHead = F2(function (game,direction) {
      var oldHead = getHead(game);
      var newHead = function () {
         var _p15 = direction;
         switch (_p15.ctor)
         {case "Up": return _U.update(oldHead,{y: oldHead.y + 1});
            case "Right": return _U.update(oldHead,{x: oldHead.x + 1});
            case "Down": return _U.update(oldHead,{y: oldHead.y - 1});
            default: return _U.update(oldHead,{x: oldHead.x - 1});}
      }();
      return wrapPoint(newHead);
   });
   var tickPlay = function (game) {
      var direction$ = changeDirection(game);
      var head$ = A2(moveHead,game,direction$);
      return A2(collisionTest,head$,game.snake) ? _U.update(game,
      {mode: Dead(0)
      ,sounds: A2($List._op["::"],
      "snake-died",
      game.sounds)}) : tickSnake(tickFood(tickBonus(_U.update(game,
      {snake: A2($List._op["::"],head$,game.snake)
      ,direction: direction$}))));
   };
   var tickGame = function (game) {
      ...
      function () {
         var _p16 = game.mode;
         switch (_p16.ctor)
         {case "Play": return tickPlay(game);
            case "Dead": return A2(tickDead,game,_p16._0);
            default: return game;}
      }());
   };
   var updateGame = F2(function (input,game) {
      return triggerSounds(function () {
         var _p17 = input;
         _v9_6: do {
            switch (_p17.ctor)
            {case "Arrows": return _U.update(game,{arrows: _p17._0});
               case "Wasd": return _U.update(game,{arrows: _p17._0});
               case "Tick": return tickGame(game);
               case "Space": if (_p17._0 === true) {
                       return changeGameMode(game);
                    } else {
                       break _v9_6;
                    }
               case "Window": if (_p17._0.ctor === "_Tuple2") {
                       return _U.update(game,
                       {window: {ctor: "_Tuple2",_0: _p17._0._0,_1: _p17._0._1}});
                    } else {
                       break _v9_6;
                    }
               case "StartTime": return _U.update(game,
                 {seed: $Random.initialSeed($Basics.round(_p17._0))});
               default: break _v9_6;}
         } while (false);
         return game;
      }());
   });
   return _elm.Snake.values = {_op: _op
                              ,deathFlashCount: deathFlashCount
                              ,gameWidth: gameWidth
                              ,gameHeight: gameHeight
                              ,tickFps: tickFps
                              ,initialGame: initialGame
                              ,updateGame: updateGame
                              ,activeBonusTicks: activeBonusTicks
                              ,getHead: getHead
                              ,getTail: getTail
...
Elm.View.make = function (_elm) {
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Snake = Elm.Snake.make(_elm),
   $Text = Elm.Text.make(_elm);
   var _op = {};
   var textHeight = 30;
   var textStyle = function (color) {
      return {typeface: _U.list(["monospace"])
             ,height: $Maybe.Just(textHeight)
             ,color: color
             ,bold: true
             ,italic: false
             ,line: $Maybe.Nothing};
   };
   var styledText = F2(function (color,string) {
      return $Graphics$Collage.text(A2($Text.style,
      textStyle(color),
      $Text.fromString(string)));
   });
   var textColor = A3($Color.rgb,250,250,150);
   var gameText = function (game) {
      var _p0 = function () {
         var _p1 = game.mode;
         switch (_p1.ctor)
         {case "NewGame": return {ctor: "_Tuple2"
                                 ,_0: "SNAKE"
                                 ,_1: "PRESS SPACE TO PLAY"};
            case "Pause": return {ctor: "_Tuple2"
                                 ,_0: "PAUSED"
                                 ,_1: "PRESS SPACE TO CONTINUE"};
            case "GameOver": return {ctor: "_Tuple2"
                                    ,_0: "GAME OVER"
                                    ,_1: "PRESS SPACE TO RETRY"};
            default: return {ctor: "_Tuple2",_0: "",_1: ""};}
      }();
      var first = _p0._0;
      var second = _p0._1;
      return _U.list([A2($Graphics$Collage.move,
                     {ctor: "_Tuple2",_0: 0,_1: textHeight},
                     A2(styledText,textColor,first))
                     ,A2($Graphics$Collage.move,
                     {ctor: "_Tuple2",_0: 0,_1: 0 - textHeight},
                     A2(styledText,textColor,second))]);
   };
   var bonusColor = A3($Color.rgb,220,100,50);
   var foodColor = A3($Color.rgb,200,200,100);
   var headColor = A3($Color.rgb,60,160,60);
   var snakeColor = A3($Color.rgb,100,200,100);
   var backColor = A3($Color.rgb,0,100,0);
   var cellSize = 16;
   var makeCell = F4(function (point,width,height,color) {
      var offset = $Basics.toFloat(cellSize) / 2;
      var x$ = $Basics.toFloat(width) / 2 - offset;
      var y$ = $Basics.toFloat(height) / 2 - offset;
      return A2($Graphics$Collage.move,
      {ctor: "_Tuple2"
      ,_0: $Basics.toFloat(point.x) * cellSize - x$
      ,_1: $Basics.toFloat(point.y) * cellSize - y$},
      A2($Graphics$Collage.filled,
      color,
      A2($Graphics$Collage.rect,cellSize,cellSize)));
   });
   var gameLayer = F3(function (width,height,game) {
      var tail = A2($List.map,
      function (point) {
         return A4(makeCell,point,width,height,snakeColor);
      },
      $Snake.getTail(game));
      var head = A4(makeCell,
      $Snake.getHead(game),
      width,
      height,
      headColor);
      var snake = function () {
         ...
         if (_p2.ctor === "Dead") {
               ...
               _p2._0,
               $Snake.deathFlashCount * 2),
               $Snake.deathFlashCount) > -1 ? A2($List._op["::"],
               head,
               tail) : _U.list([]);
            } else {
               return A2($List._op["::"],head,tail);
            }
      }();
      var bonus = function () {
         var _p3 = game.bonus.point;
         if (_p3.ctor === "Just") {
               return $Maybe.Just(A4(makeCell,
               _p3._0,
               width,
               height,
               bonusColor));
      ...
      var food = A4(makeCell,game.food,width,height,foodColor);
      var _p4 = bonus;
      if (_p4.ctor === "Just") {
            return A2($List._op["::"],
            food,
            A2($List._op["::"],_p4._0,snake));
         } else {
            return A2($List._op["::"],food,snake);
         }
   });
   var view = F2(function (_p5,game) {
      var bonusString = function () {
         var _p6 = game.bonus.point;
         if (_p6.ctor === "Just") {
               var x = $Snake.activeBonusTicks / 10 | 0;
               return $Basics.toString((game.bonus.ticks + x - 1) / x | 0);
            } else {
               return "";
            }
      }();
      var height = $Snake.gameHeight * cellSize;
      var width = $Snake.gameWidth * cellSize;
      var background = A2($Graphics$Collage.filled,
      backColor,
      A2($Graphics$Collage.rect,width,height));
      var score = A2($Graphics$Collage.move,
      {ctor: "_Tuple2"
      ,_0: $Basics.toFloat(width) / 2 - 30
      ,_1: $Basics.toFloat(height) / 2 - 20},
      A2(styledText,textColor,$Basics.toString(game.score)));
      var bonus = A2($Graphics$Collage.move,
      {ctor: "_Tuple2"
      ,_0: $Basics.toFloat(30) - width / 2
      ,_1: $Basics.toFloat(height) / 2 - 20},
      A2(styledText,bonusColor,bonusString));
      return $Html.fromElement(A4($Graphics$Element.container,
      $Basics.fst(game.window),
      $Basics.snd(game.window),
      ...
      function () {
         var _p7 = game.mode;
         switch (_p7.ctor)
         {case "NewGame": return A2($List._op["::"],
              background,
              gameText(game));
            case "Pause": return A2($List._op["::"],
              background,
              A2($List._op["::"],score,gameText(game)));
            case "GameOver": return A2($List._op["::"],
              background,
              A2($List._op["::"],score,gameText(game)));
            default: return $List.concat(_U.list([A2($List._op["::"],
                                                 background,
                                                 A3(gameLayer,width,height,game))
                                                 ,_U.list([score,bonus])]));}
      }())));
   });
...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var playSound = Elm.Native.Port.make(_elm).outboundSignal("playSound",
   function (v) {
      return v;
   },
   $Snake.soundSignal);
   var inputSignals = _U.list([A2($Signal.map,
                              $Snake.Tick,
                              $Time.fps($Snake.tickFps))
                              ,A2($Signal.map,$Snake.Arrows,$Keyboard.arrows)
                              ,A2($Signal.map,$Snake.Wasd,$Keyboard.wasd)
                              ,A2($Signal.map,$Snake.Space,$Keyboard.space)
                              ,A2($Signal.map,
                              $Snake.Window,
                              $Primer.prime($Window.dimensions))]);
   var app = $StartApp.start({init: $Snake.initialGame
